...
void AnalogJoystick::UpdateInput( Vec2 rawIn ) {
    ...
    tempXY.x = RangeMapFloat( m_rawPosition.x, -32768, 32767, -1.f, 1.f );
    tempXY.y = RangeMapFloat( m_rawPosition.y, -32768, 32767, -1.f, 1.f );
    ...
    if( tempXY.GetLength() < m_innerDeadzoneFraction ) {
        ...
        float correctedLength = RangeMapFloat( tempXY.GetLength(), m_innerDeadzoneFraction, m_outerDeadzoneFraction, 0.f, 1.f );
