...
SoundID AudioSystem::CreateOrGetSound( const std::string& soundFilePath )
{
	...
	{
        g_theDevConsole->PrintString( Stringf( "(AudioSystem) Loading new sound from file (%s)...", soundFilePath.c_str() ), Rgba::YELLOW );
		...
		if( newSound )
		{
            ...
            g_theDevConsole->PrintString( "(AudioSystem) Loading complete", Rgba::YELLOW );
			return newSoundID;
		}

        g_theDevConsole->PrintString( "(AudioSystem) WARNING: MISSING_SOUND_ID", Rgba::RED );
	}
