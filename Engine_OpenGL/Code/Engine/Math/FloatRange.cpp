...
FloatRange::FloatRange( const char* initialRange ) {
...
void FloatRange::SetFromText( const char* valuesAsText ) {
    std::vector<std::string> splitString = SplitStringOnDelimeter( valuesAsText, '~' );

    int numValues = (int)splitString.size();
    GUARANTEE_OR_DIE( numValues == 1 || numValues == 2, Stringf( "Invalid initialization string (%s) for FloatRange", valuesAsText ) );
