...
void AddVertsForDisc2D( std::vector<Vertex_PCU>& vertexArray, const Vec2& center, float radius, const Rgba& color, int numSides /*= 64 */ ) {
    ...
    for( int i = 0; i < numSides; i++ ) {
        // Triangle Vert1
        vertexArray.push_back( Vertex_PCU( Vec3( center.x, center.y, 0.f ), color, Vec2( 0.f, 0.f ) ) );

        // Triangle Vert2
        transformedOuterVert = TransformPosition( initialOuterVert, radius, vertAngle, center );
        vertexArray.push_back( Vertex_PCU( transformedOuterVert, color, Vec2( 0.f, 0.f ) ) );
...
void AddVertsForLine2D( std::vector<Vertex_PCU>& vertexArray, const Vec2& start, const Vec2& end, float thickness, const Rgba& color ) {
    ...
    vertexArray.push_back( Vertex_PCU( Vec3( startLeft.x, startLeft.y, 0.f ), color, Vec2( 0.f, 0.f ) ) );
    vertexArray.push_back( Vertex_PCU( Vec3( startRight.x, startRight.y, 0.f ), color, Vec2( 0.f, 0.f ) ) );
    vertexArray.push_back( Vertex_PCU( Vec3( endLeft.x, endLeft.y, 0.f ), color, Vec2( 0.f, 0.f ) ) );

    vertexArray.push_back( Vertex_PCU( Vec3( startRight.x, startRight.y, 0.f ), color, Vec2( 0.f, 0.f ) ) );
    vertexArray.push_back( Vertex_PCU( Vec3( endLeft.x, endLeft.y, 0.f ), color, Vec2( 0.f, 0.f ) ) );
    vertexArray.push_back( Vertex_PCU( Vec3( endRight.x, endRight.y, 0.f ), color, Vec2( 0.f, 0.f ) ) );
}
...
void AddVertsForRing2D( std::vector<Vertex_PCU>& vertexArray, const Vec2& center, float radius, float thickness, const Rgba& color, int numSides /*= 64 */ ) {
    ...
    for( int i = 0; i < numSides; i++ ) {
        ...
        vertexArray.push_back( Vertex_PCU( innerRight, color, Vec2( 0.f, 0.f ) ) );

        // Triangle Vert2
        outerRight = TransformPosition( identityVector, outerRadius, vertAngle, center );
        vertexArray.push_back( Vertex_PCU( outerRight, color, Vec2( 0.f, 0.f ) ) );

        // Triangle Vert3
        vertAngle += degreesPerTriangle;
        innerLeft = TransformPosition( identityVector, innerRadius, vertAngle, center );
        vertexArray.push_back( Vertex_PCU( innerLeft, color, Vec2( 0.f, 0.f ) ) );
        ...
        vertexArray.push_back( Vertex_PCU( outerLeft, color, Vec2( 0.f, 0.f ) ) );
    }
}
...
void AddVertsForAABB2D( std::vector<Vertex_PCU>& vertexArray, const AABB2& box, const Rgba& color, const Vec2& uvAtMins /*= Vec2( 0.f, 0.f )*/, const Vec2& uvAtMaxs /*= Vec2( 1.f, 1.f ) */ ) {
    ...
    vertexArray.push_back( Vertex_PCU( Vec3( box.mins.x, box.mins.y, 0.f ), color, uvAtMins ) );
    vertexArray.push_back( Vertex_PCU( Vec3( box.mins.x, box.maxs.y, 0.f ), color, uvTL ) );
    vertexArray.push_back( Vertex_PCU( Vec3( box.maxs.x, box.mins.y, 0.f ), color, uvBR ) );

    vertexArray.push_back( Vertex_PCU( Vec3( box.mins.x, box.maxs.y, 0.f ), color, uvTL ) );
    vertexArray.push_back( Vertex_PCU( Vec3( box.maxs.x, box.mins.y, 0.f ), color, uvBR ) );
    vertexArray.push_back( Vertex_PCU( Vec3( box.maxs.x, box.maxs.y, 0.f ), color, uvAtMaxs ) );
}


void TransformVertex( Vertex_PCU& position, float scaleXY, float rotationDegrees, const Vec2& translationXY ) {
    position.m_position = TransformPosition( position.m_position, scaleXY, rotationDegrees, translationXY );
}


void TransformVertexArray( int numVertices, Vertex_PCU* position, float scaleXY, float rotationDegrees, const Vec2& translationXY ) {
...
void TransformVertexArray( std::vector<Vertex_PCU>& vertexArray, float scaleXY, float rotationDegreesAboutZ, const Vec2& translationXY ) {
    ...
    Vertex_PCU* data = vertexArray.data();
