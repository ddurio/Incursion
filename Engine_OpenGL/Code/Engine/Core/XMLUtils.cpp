...
const XMLElement& ParseXMLRootElement( const char* fileName, XMLDocument& document ) {
    //tinyxml2::XMLDocument document = new tinyxml2::XMLDocument();
    tinyxml2::XMLError loadSuccess = document.LoadFile( fileName );
    GUARANTEE_OR_DIE( loadSuccess == tinyxml2::XML_SUCCESS, Stringf("Failed to load XML file: %s", fileName) );

    tinyxml2::XMLNode* root = document.FirstChild();
    GUARANTEE_OR_DIE( root != nullptr, Stringf( "Poorly constructed XML file: %s", fileName ) );
