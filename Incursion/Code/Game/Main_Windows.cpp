...
const char* APP_NAME = "Incursion Foundation (ddurio)";	// ...becomes ???
...
LRESULT CALLBACK WindowsMessageHandlingProcedure( HWND windowHandle, UINT wmMessageCode, WPARAM wParam, LPARAM lParam ) {
	switch( wmMessageCode ) {
		...
		case WM_KEYDOWN:
		{
			//if( !( HIWORD(lParam) & KF_REPEAT ) ) { // Disable Key Repeats
...
void Startup( HINSTANCE applicationInstanceHandle ) {
    const char* projectConfigFilePath = "Data/ProjectConfig.xml";
    tinyxml2::XMLDocument projectConfig = new tinyxml2::XMLDocument();
    tinyxml2::XMLError loadSuccess = projectConfig.LoadFile( projectConfigFilePath );
    GUARANTEE_OR_DIE( loadSuccess == tinyxml2::XML_SUCCESS, Stringf( "Failed to load XML file: %s", projectConfigFilePath ) );
